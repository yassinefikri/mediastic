...
composer() {
  docker exec -it --user app symfony-vue_php-apache2_1 sh -c "COMPOSER_MEMORY_LIMIT=-1 composer $*"
}
export -f composer

unalias console 2>/dev/null >/dev/null || true
console() {
  docker exec -it --user app symfony-vue_php-apache2_1 sh -c "php bin/console $*"
}
export -f console
...
phpstan() {
  docker exec -it --user app symfony-vue_php-apache2_1 sh -c "vendor/bin/phpstan analyse src tests --level 8"
}
export -f phpstan

unalias dump-routes 2>/dev/null >/dev/null || true
dump-routes() {
  docker exec -it --user app symfony-vue_php-apache2_1 sh -c "php bin/console fos:js-routing:dump --format=json --target=public/js/fos_js_routes.json"
}
